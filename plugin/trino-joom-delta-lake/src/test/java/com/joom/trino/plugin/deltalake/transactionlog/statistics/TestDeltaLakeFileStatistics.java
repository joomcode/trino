 ...
 */
package com.joom.trino.plugin.deltalake.transactionlog.statistics;

import com.fasterxml.jackson.databind.ObjectMapper;
import com.google.common.collect.ImmutableSet;
import com.joom.trino.plugin.deltalake.DeltaLakeColumnHandle;
import com.joom.trino.plugin.deltalake.transactionlog.DeltaLakeTransactionLogEntry;
import com.joom.trino.plugin.deltalake.transactionlog.MetadataEntry;
import com.joom.trino.plugin.deltalake.transactionlog.checkpoint.CheckpointEntryIterator;
...
import static com.joom.trino.plugin.deltalake.transactionlog.checkpoint.CheckpointEntryIterator.EntryType.METADATA;
...
public class TestDeltaLakeFileStatistics
{
    ...
    private static void testStatisticsValues(DeltaLakeFileStatistics fileStatistics)
    {
        ...
        assertEquals(
                fileStatistics.getMinColumnValue(new DeltaLakeColumnHandle("byt", TINYINT, REGULAR, Optional.empty())),
                Optional.of(42L));
        assertEquals(
                fileStatistics.getMinColumnValue(new DeltaLakeColumnHandle("dat", DATE, REGULAR, Optional.empty())),
                Optional.of(LocalDate.parse("5000-01-01").toEpochDay()));
        assertEquals(
                fileStatistics.getMinColumnValue(new DeltaLakeColumnHandle("dec_long", DecimalType.createDecimalType(25, 3), REGULAR, Optional.empty())),
                Optional.of(encodeScaledValue(new BigDecimal("999999999999.123"), 3)));
        assertEquals(
                fileStatistics.getMinColumnValue(new DeltaLakeColumnHandle("dec_short", DecimalType.createDecimalType(5, 1), REGULAR, Optional.empty())),
                Optional.of(new BigDecimal("10.1").unscaledValue().longValueExact()));
        assertEquals(
                fileStatistics.getMinColumnValue(new DeltaLakeColumnHandle("dou", DoubleType.DOUBLE, REGULAR, Optional.empty())),
                Optional.of(0.321));
        assertEquals(
                fileStatistics.getMinColumnValue(new DeltaLakeColumnHandle("fl", REAL, REGULAR, Optional.empty())),
                Optional.of((long) floatToIntBits(0.123f)));
        assertEquals(
                fileStatistics.getMinColumnValue(new DeltaLakeColumnHandle("in", INTEGER, REGULAR, Optional.empty())),
                Optional.of(20000000L));
        assertEquals(
                fileStatistics.getMinColumnValue(new DeltaLakeColumnHandle("l", BIGINT, REGULAR, Optional.empty())),
        ...
        assertEquals(
                fileStatistics.getMinColumnValue(new DeltaLakeColumnHandle("row", rowType, REGULAR, Optional.empty())),
                Optional.empty());
        assertEquals(
                fileStatistics.getMinColumnValue(new DeltaLakeColumnHandle("arr", new ArrayType(INTEGER), REGULAR, Optional.empty())),
                Optional.empty());
        assertEquals(
                fileStatistics.getMinColumnValue(new DeltaLakeColumnHandle("m", new MapType(INTEGER, VarcharType.createUnboundedVarcharType(), new TypeOperators()), REGULAR, Optional.empty())),
                Optional.empty());
        assertEquals(
                fileStatistics.getMinColumnValue(new DeltaLakeColumnHandle("sh", SMALLINT, REGULAR, Optional.empty())),
                Optional.of(123L));
        assertEquals(
                fileStatistics.getMinColumnValue(new DeltaLakeColumnHandle("str", VarcharType.createUnboundedVarcharType(), REGULAR, Optional.empty())),
                Optional.of(utf8Slice("a")));
        assertEquals(
                fileStatistics.getMinColumnValue(new DeltaLakeColumnHandle("ts", TIMESTAMP_TZ_MILLIS, REGULAR, Optional.empty())),
                Optional.of(packDateTimeWithZone(LocalDateTime.parse("2960-10-31T01:00:00.000").toInstant(UTC).toEpochMilli(), UTC_KEY)));
        assertEquals(
                fileStatistics.getMinColumnValue(new DeltaLakeColumnHandle("bool", BOOLEAN, REGULAR, Optional.empty())),
                Optional.empty());
        assertEquals(
                fileStatistics.getMinColumnValue(new DeltaLakeColumnHandle("bin", VARBINARY, REGULAR, Optional.empty())),
                Optional.empty());
    }
}
