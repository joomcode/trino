...
import static com.joom.trino.plugin.deltalake.transactionlog.TransactionLogParser.LAST_CHECKPOINT_FILENAME;
import static com.joom.trino.plugin.deltalake.transactionlog.TransactionLogUtil.TRANSACTION_LOG_DIRECTORY;
...
public class TestTransactionLogAccess
{
            ...
            throws Exception
    {
        ...
        for (int i = 0; i < expectedDataFiles.size(); i++) {
            ...
            for (ColumnMetadata column : columns) {
                DeltaLakeColumnHandle columnHandle = new DeltaLakeColumnHandle(column.getName(), column.getType(), REGULAR, Optional.empty());
            ...
            throws Exception
    {
        ...
        for (String columnName : statsValues.keySet()) {
            ...
            assertEquals(
                    fileStats.getMinColumnValue(new DeltaLakeColumnHandle(columnName, IntegerType.INTEGER, REGULAR, Optional.empty())),
                    Optional.of(statsValues.get(columnName)));

            assertEquals(
                    fileStats.getMaxColumnValue(new DeltaLakeColumnHandle(columnName, IntegerType.INTEGER, REGULAR, Optional.empty())),
                    Optional.of(statsValues.get(columnName)));
        }
    }
